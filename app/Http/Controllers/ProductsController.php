...
class ProductsController extends Controller
{
    ...
    public function fetch_all_products_by_vendor_id($vendor_id){
        $fetch_all_products_by_vendor = Products::where('vendor',$vendor_id)->orderby('id','desc')->get();

        if(!$fetch_all_products_by_vendor){
            $this->result->status = true;
            $this->result->status_code = 400;
            $this->result->message = "An Error Ocurred, we couldn't fetch all the products for the vendor";
            return response()->json($this->result);
        }
